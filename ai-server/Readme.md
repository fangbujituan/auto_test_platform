...
项目内置完整的 Token 用量追踪和用例管理系统：

1. **TokenCounter**（`tools/utils/token_counter.py`）：
   - 单例模式，SQLite 持久化存储（`data/token_stats.db`）
   - 自动记录每次 LLM 调用的 input/output tokens
   - 支持用例级追踪：将多轮对话的 token 消耗关联到同一用例
   - 费用估算：根据模型和提供商计算预估费用
   - 趋势分析：按天/小时粒度统计用量趋势

2. **TokenCallbackHandler**：
   - LangChain 回调处理器，自动挂载到所有模型调用
   - 在 `get_default_model()` 中自动注入

3. **用例管理工具**（`tools/utils/case_tools.py`）：
   - `case_create`: 创建测试用例，后续 token 自动关联
   - `case_get_stats`: 获取用例当前统计（本次消耗、总计、费用）
   - `case_complete`: 完成用例并返回最终统计
   - `case_rename`: 重命名用例
   - `case_list`: 获取历史用例列表

4. **Dashboard API**（`tools/utils/dashboard.py`）：
   - 用例 CRUD 操作
   - 概览统计（今日/本周/本月）
   - 趋势图数据
   - 费用报表
