...
test('add_billing_item_test', async ({ page }) => {
  ...
  // 选择 Billing Category - ACCESSORIAL（精确匹配）
  await page.locator('.el-form-item').filter({ hasText: 'Billing Category' }).locator('input').click();
  await page.waitForTimeout(5000);
  await page.locator('.el-select-dropdown__item').getByText('ACCESSORIAL', { exact: true }).click();
  ...
  // 选择 Rate Type - Unit Prn  await page.locator('.el-form-item').filter({ hasText: 'Rate Type' }).locator('input').click();
  await page.waitForTimeout(5000);
