...
通过 ``TestCaseLoader`` 完成"用例 → spec"的具体转换。

设计要点：
- 找不到任务抛 ``LoaderError``
- 关联用例不存在时**不**报错，而是产出一个标记"loader_missing"的 step（与老
  ``AutomationExecutor`` 行为一致：用例不存在记录为 error 但不中断整体）
- ``automation_task_cases.case_mgmt_id`` 字段（测试用例管理）当前 Phase 不处理；
  仅消费 ``case_id`` 关联的 ``test_cases`` 行
- 不在此处加载变量；``environment_id`` 仅写入 CollectionSpec，由 ExecutionContext 统一处理
...
class AutomationTaskLoader(BaseLoader):
    ...
    def __init__(self, test_case_loader: TestCaseLoader | None = None):
    ...
    def load_collection(
        ...
        *,
        ...
        Args:
            ...
            fail_strategy:  fail_fast / continue（默认 continue，与老 AutomationExecutor 一致）
        ...
        for tc in task_cases:
            spec = self._build_spec_for_task_case(tc)
            specs.append(spec)

        logger.info(
            "[api_engine] AutomationTaskLoader 加载完成 task_id=%s name=%s 步骤数=%d",
            task_id, task.name, len(specs),
        )
    ...
    def _build_spec_for_task_case(self, task_case) -> RequestSpec:
        """单条 ``automation_task_cases`` 行 → ``RequestSpec``。

        关联用例不存在时返回占位 spec（method=GET, url=""），让引擎在执行时产出
        一个明确失败的 step，由 reporter 落 status=error，行为与老路径对齐。
        """
        if not task_case.case_id:
            return self._missing_spec(
                reason=f"任务用例项 #{task_case.id} 未关联 case_id（可能是 case_mgmt 类型，本期暂不支持）"
            )
