...
6. 整批结束调用 ``reporter.on_collection_finished``，返回 ``CollectionResult``

设计要点：
- reporter 钩子异常**自吞** + 日志，不影响主流程
- 整批顶层 try/except，把未捕获异常落到 ``CollectionResult.error_message``
- ``iterations`` 字段当前固定走 1 次（数据驱动留口子，本期不实现）
...
class SequenceRunner:
    ...
    def run(self, spec: CollectionSpec, ctx: ExecutionContext) -> CollectionResult:
        ...
        logger.info(
            "[api_engine] run_id=%s collection=%s 开始 | strategy=%s, total=%d",
            ctx.run_id, spec.name, strategy.name, len(spec.requests),
        )
        self._fire_started(spec, ctx)

        try:
            for index, req_spec in enumerate(spec.requests):
                step_result = self._step_executor.execute(req_spec, ctx, step_index=index)
                steps.append(step_result)
                self._fire_step_completed(step_result, ctx)

                # 失败策略判断
                if not step_result.passed and strategy.should_stop(step_result, req_spec):
                    logger.warning(
                        "[api_engine] run_id=%s 触发中断 step=%d | strategy=%s, on_failure=%s",
                        ctx.run_id, index, strategy.name, req_spec.on_failure,
                    )
                    break
