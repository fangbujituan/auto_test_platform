...
整合自 ai-server/tools/middleware/base64_filter.py
"""
...
def _save_base64_image(base64_data: str, message_id: str, config) -> str:
    ...
    Args:
        ...
        config: 配置对象（需要有 workspace_root 和 base64_images_dir）
    ...
    base64_images_dir = getattr(config, 'base64_images_dir', 'data/base64_images')
    
    images_dir = Path(workspace_root) / base64_images_dir.lstrip("/")
...
class Base64FilterMiddleware(AgentMiddleware):
    ...
    def __init__(self, config=None):
        ...
        Args:
            config: 配置对象，包含 workspace_root 和 base64_images_dir（可选）
        """
        super().__init__()
        self.config = config or type('', (), {})()  # 默认空对象
        ...
        if isinstance(result, ToolMessage):
            ...
            if isinstance(original_content, str):
                ...
                if stats["lmca_format"] > 0 or stats["mcp_image_format"] > 0 or stats["json_format"] > 0 or stats["data_uri_format"] > 0 or stats["pure_format"] > 0:
                    ...
                    message_id = result.tool_call_id or getattr(result, 'id', None) or "unknown"
